if GetLocale() == "ruRU" then
	DBM_KARAZHAN            = "Каражан";
	
	--Attumen
	DBM_ATH_NAME			= "Аттумен Охотник";
	DBM_ATH_DESCRIPTION		= "Shows a timer for his curse.";
	DBM_ATH_OPTION_1		= "Show 5 second warning";

	DBM_ATH_WARN_CURSE		= "*** Курса ***";
	DBM_ATH_CURSE_SOON		= "*** скоро Курса ***";
	DBM_ATH_SUMMON			= "*** Attumen has been summoned***";
	DBM_ATH_WARN_PHASE		= "*** Фаза %s ***";

	DBM_ATH_MIDNIGHT		= "Полночь";
	DBM_ATH_CURSE			= "поражен Intangible Presence";
	DBM_ATH_YELL_1			= "Давай, Полночь, разгоним этот сброд!";


	--Moroes
	DBM_MOROES_NAME			= "Мороуз";
	DBM_MOROES_DESCRIPTION	= "Показывает таймеры для Исчезновение.";
	DBM_MOROES_OPTION_1		= "Показать предупреждение Исчезновение";
	DBM_MOROES_OPTION_2		= "Показать предупреждение \"Исчезновение рассеивается\"";
	DBM_MOROES_OPTION_3		= "Показать предупреждение \"Cкоро Исчезновение\"";
	DBM_MOROES_OPTION_4		= "Показать предупреждение Гаррота";
	DBM_MOROES_OPTION_5		= "Show gouge warning";
	DBM_MOROES_OPTION_6		= "Show blind warning";

	DBM_MOROES_VANISH_WARN	= "*** Исчезновение ***";
	DBM_MOROES_VANISH_FADED	= "*** Исчезновение рассеивается ***";
	DBM_MOROES_VANISH_SOON	= "*** Исчезновение через 5 секунд ***";
	DBM_MOROES_GARROTE_WARN	= "*** Гаррота на >%s< ***";
	DBM_MOROES_GOUGE_WARN	= "*** Gouge on >%s< ***";
	DBM_MOROES_BLIND_WARN	= "*** Blind on >%s< ***";

	DBM_MOROES_YELL_START	= "Хмм, неожиданные посетители. Нужно подготовиться...";
	DBM_MOROES_VANISH_GAIN	= "Мороуз читает Исчезновение.";
	DBM_MOROES_VANISH_FADES	= "Эффект Исчезновение рассеивается.";
	DBM_MOROES_GARROTE		= "([^%s]+) (%w+) под воздействием эффекта Исчезновение%.";



	-- Maiden of Virtue
	DBM_MOV_NAME			= "Благочестивая дева";
	DBM_MOV_DESCRIPTION		= "Объявляет и показывает таймер для Покаяние, Священный огонь и область границы контроля.";
	DBM_MOV_OPTION_1		= "Показать область границы контроля";
	DBM_MOV_OPTION_2		= "Объявить Священный огонь";
	DBM_MOV_OPTION_3		= "Announce Repentance";

	DBM_MOV_YELL_PULL		= "Ваше поведение нестерпимо.";
	DBM_MOV_YELL_REP_1		= "Отбросьте свои нечистые помыслы.";
	DBM_MOV_YELL_REP_2		= "Нужно очистить мир от вас!";
	DBM_MOV_REPENTANCE		= "находится под воздействием эффекта Покаяние";
	DBM_MOV_WARN_REP		= "*** Покаяние ***";
	DBM_MOV_WARN_REP_SOON	= "*** Скоро Покаяние ***";

	DBM_MOV_WARN_HOLYFIRE	= "*** >%s< под воздействием эффекта Священный огонь ***";

	-- Romulo and Julianne
	DBM_RJ_NAME				= "Ромуло и Джулианна";
	DBM_RJ_DESCRIPTION		= "Объявляет Бесстрашие Ромуло и Преданность Джулианны.";
	DBM_RJ_OPTION_1			= "Объявить Исцеление Джулианны";
	DBM_RJ_OPTION_2			= "Объявить Ядовитый укол Ромуло";

	DBM_RJ_DARING_WARN		= "*** Бесстрашие Ромуло ***";
	DBM_RJ_DEVOTION_WARN	= "*** Преданность Джулианны ***";
	DBM_RJ_HEAL_WARN		= "*** Исцеление ***";
	DBM_RJ_POISON_WARN		= ">%s< под воздействием эффекта Ядовитый укол";

	DBM_RJ_ROMULO			= "Ромуло";
	DBM_RJ_JULIANNE			= "Джулианна";
	DBM_RJ_PHASE2_YELL		= "Ночь, добрая и строгая, приди! Верни мне моего Ромуло!";


	-- Big Bad Wolf
	DBM_BBW_NAME			= "Злой и страшный серый волк";
	DBM_BBW_DESCRIPTION		= "Объявляет и показывает таймер для \"Красная Шапочка\".";
	DBM_BBW_OPTION_1		= "Объявить Страх";
	DBM_BBW_OPTION_2		= "Отправить сообщение шепотом";

	DBM_BBW_YELL_1			= "Так вам и надо!";
	DBM_BBW_FEAR_EXP		= "Ужасающий вой";

	DBM_BBW_FEAR_WARN		= "*** Страх ***";
	DBM_BBW_FEAR_SOON		= "*** Скоро Страх ***";
	DBM_BBW_RRH_WARN		= "*** >%s< - маленькая Красная Шапочка ***";
	DBM_BBW_RUN_AWAY		= "Беги!";
	DBM_BBW_RUN_AWAY_WHISP	= "Ты маленькая Красная Шапочка! Беги!";
	DBM_BBW_RRH_SOON_WARN	= "*** Скоро Красная Шапочка ***";

	-- Curator
	DBM_CURA_NAME			= "Смотритель";
	DBM_CURA_DESCRIPTION	= "Показывает таймеры для Прилива сил.";

	DBM_CURA_YELL_PULL		= "Галерея только для гостей.";
	DBM_CURA_YELL_OOM		= "Ваш запрос не может быть выполнен.";

	DBM_CURA_EVO_NOW		= "*** Прилив сил ***";
	DBM_CURA_EVO_SOON		= "*** Скоро Прилив сил ***";
	DBM_CURA_EVO_1MIN		= "Прилив сил через 1 минуту"

	DBM_SBT["Next Evocation"]	= "Следующий Прилив сил";
	DBM_SBT["Evocation"]		= "Прилив сил";

	-- Terestian Illhoof
	DBM_TI_NAME				= "Терестиан Больное Копыто";
	DBM_TI_DESCRIPTION		= "Объявляет Демонические цепи и фазы Ослабления.";
	DBM_TI_OPTION_1			= "Объявить появление беса";

	DBM_TI_YELL_PULL		= "А, вы как раз вовремя! Ритуалы скоро начнутся!";
	DBM_TI_SACRIFICE		= "([^%s]+) (%w+) под воздействием эффекта Жертвоприношение%.";
	DBM_TI_EMOTE_IMP		= "%s shrieks in pain and points at his master.";
	DBM_TI_CAST_IMP			= "Терестиан Больное Копыто призывает беса.";

	DBM_TI_SACRIFICE_WARN	= "*** >%s< в Демонических цепях ***";
	DBM_TI_SACRIFICE_SOON	= "*** Скоро Жертвоприношение ***";
	DBM_TI_WEAKENED_WARN	= "*** Ослабление ***";
	DBM_TI_IMP_SOON			= "*** Скоро появится бес ***";
	DBM_TI_IMP_RESPAWNED	= "*** Бес появился ***";

	-- Shade of Aran
	DBM_ARAN_NAME			= "Тень Арана";
	DBM_ARAN_DESCRIPTION	= "Объявляет Венец Пламени и Волшебный взрыв.";

	DBM_ARAN_CAST_WREATH	= "Тень Арана начинает читать Венец Пламени.";
	DBM_ARAN_CAST_AE		= "Тень Арана начинает читать Волшебный взрыв.";
	DBM_ARAN_CAST_BLIZZ		= "Тень Арана начинает призывать Снежную бурю.";
	DBM_ARAN_YELL_ADDS		= "Со мной еще не покончено! Нет, у меня есть еще туз в рукаве...";
	DBM_ARAN_YELL_BLIZZ1	= "Отправляйся в холодную тьму!";
	DBM_ARAN_YELL_BLIZZ2	= "Всех заморожу!";

	DBM_ARAN_WREATH_WARN	= "*** Венец Пламени через 5 секунд ***";
	DBM_ARAN_AE_WARN		= "*** Волшебный взрыв ***";
	DBM_ARAN_BLIZZ_WARN		= "*** Снежная буря ***";
	DBM_ARAN_ADDS_WARN		= "*** Элементали ***";
	DBM_ARAN_DO_NOT_MOVE	= "Не двигайтесь!";

	DBM_SBT["Flame Wreath Cast"]		= "Заклинание Венец Пламени";
	DBM_SBT["Flame Wreath"]				= "Венец Пламени";
	DBM_SBT["Arcane Explosion"]			= "Волшебный взрыв";
	DBM_SBT["Blizzard"]					= "Снежная буря";
	DBM_SBT["Elementals despawn in"]	= "Элементали появятся через";

	--Netherspite
	DBM_NS_NAME				= "Пустогнев";
	DBM_NS_DESCRIPTION		= "Объявляет и показывает таймер для Дыхание Хаоса и Портал Бездны.";
	DBM_NS_OPTION_1			= "Объявить фазы";
	DBM_NS_OPTION_2			= "Показать 5 секундное предупреждение перед фазами";
	DBM_NS_OPTION_3			= "Объявить Портал Бездны";
	DBM_NS_OPTION_4			= "Объявить Дыхание Хаоса";

	DBM_NS_EMOTE_PHASE_2	= "%s goes into a nether-fed rage!";
	DBM_NS_EMOTE_PHASE_1	= "%s cries out in withdrawal, opening gates to the nether.";

	DBM_NS_WARN_PORTAL		= "*** Фаза порталов ***";
	DBM_NS_WARN_BANISH		= "*** Фаза изгнания ***";
	DBM_NS_WARN_PORTAL_SOON	= "*** Фаза порталов через 5 секунд ***";
	DBM_NS_WARN_BANISH_SOON	= "*** Фаза изгнания через 5 секунд ***";
	DBM_NS_WARN_BREATH		= "*** Дыхание Хаоса ***";
	DBM_NS_WARN_VOID_ZONE	= "*** Портал Бездны ***";
	DBM_NS_WARN_ENRAGE		= "*** Ярость через %s %s ***";

	DBM_SBT["Portal Phase"]	= "Фаза порталов";
	DBM_SBT["Banish Phase"]	= "Фаза изгнания";
	DBM_SBT["Netherbreath"]	= "Дыхание Хаоса";
	DBM_SBT["Enrage"]		= "Ярость";

	--Prince Malchezaar
	DBM_PRINCE_NAME			= "Принц Малчезар"
	DBM_PRINCE_DESCRIPTION	= "Объявляет Инферналы, Ослабление, Слово Тьмы: Боль и Кольцо Тьмы.";
	DBM_PRINCE_OPTION_1		= "Объявить Кольцо Тьмы";
	DBM_PRINCE_OPTION_2		= "Объявить Ослабление";
	DBM_PRINCE_OPTION_3		= "Отправить сообщение шепотом";
	DBM_PRINCE_OPTION_4		= "Объявить Слово Тьмы: Боль";
	DBM_PRINCE_OPTION_5		= "Объявить Инфернал";

	DBM_PRINCE_YELL_PULL	= "Безумие привело вас сюда, ко мне. Я стану вашей погибелью!";
	DBM_PRINCE_YELL_P2		= "Простофили! Время – это пламя, в котором вы сгорите!";
	DBM_PRINCE_YELL_P3		= "Как ты можешь надеяться выстоять против такой ошеломляющей мощи?";
	DBM_PRINCE_YELL_INF1	= "Все миры, все измерения открыты мне!";
	DBM_PRINCE_YELL_INF2	= "Вас ждет не один Малчезаар, но и легионы, которыми я командую!";

	DBM_PRINCE_WARN_NOVA		= "*** Кольцо Тьмы через 2 секунды ***";
	DBM_PRINCE_WARN_ENFEEBLE	= "*** Ослабление ***";
	DBM_PRINCE_WHISP_ENFEEBLE	= "Ты под воздействием эффекта Ослабление!";
	DBM_PRINCE_WARN_SWP			= "*** >%s< под воздействием эффекта Слово Тьмы: Боль ***";
	DBM_PRINCE_WARN_INF			= "*** Инфернал #%s ***";
	DBM_PRINCE_WARN_INF_SOON	= "*** Скоро Инфернал #%s ***"
	DBM_PRINCE_WARN_PHASE		= "*** Фаза %s ***";

	DBM_SBT["Infernal"]		= "Инфернал";
	DBM_SBT["Enfeeble"]		= "Ослабление";
	DBM_SBT["Shadow Nova"]	= "Кольцо Тьмы";

	-- Nightbane
	DBM_NB_NAME				= "Ночная Погибель";
	DBM_NB_DESCRIPTION		= "Объявляет Страх, Опаленная земля, Отвлекающие угли, Костяной дождь, Дымовой заряд и воздушную фазу.";
	DBM_NB_OPTION_1			= "Показать спец-предупреждени для Опаленная земля";
	DBM_NB_OPTION_2			= "Объявить Отвлекающие угли";
	DBM_NB_OPTION_3			= "Объявить Костяной дождь";
	DBM_NB_OPTION_4			= "Объявить Дымовой заряд";
	DBM_NB_OPTION_5			= "Показать спец-предупреждени когда Ночная Погибель читает Дымовой заряд на вас";
	DBM_NB_OPTION_ICON		= "Установить метку на цель Костяной дождь";

	DBM_NB_EMOTE_PULL		= "Древнее существо пробуждается вдалеке…";
	DBM_NB_YELL_PULL		= "Ну и глупцы! Я быстро покончу с вашими страданиями!";
	DBM_NB_YELL_AIR			= "Жалкий гнус! Я изгоню тебя из воздуха!";
	DBM_NB_YELL_GROUND		= "Довольно! Я сойду на землю и сам раздавлю тебя!";
	DBM_NB_YELL_GROUND2		= "Ничтожества! Я вам покажу мою силу поближе!";

	DBM_NB_FEAR_WARN		= "*** Страх ***";
	DBM_NB_FEAR_SOON_WARN	= "*** Скоро Страх ***";
	DBM_NB_AIR_WARN			= "*** Воздушная фаза ***";
	DBM_NB_EARTH_WARN		= "Опаленная земля";
	DBM_NB_SMOKE_SPECWARN	= "Дымовой заряд";
	DBM_NB_BONES_WARN		= "*** Костяной дождь ***";
	DBM_NB_ASH_WARN			= "*** >%s< под воздействием эффекта Отвлекающие угли ***";
	DBM_NB_SMOKE_WARN		= "*** >%s< под воздействием эффекта Дымовой заряд ***";
	DBM_NB_DOWN_WARN 		= "*** Наземная фаза через 15 секунд ***";
	DBM_NB_DOWN_WARN2 		= "*** Наземная фаза через 5 секунд ***";

	-- Wizard of Oz
	DBM_OZ_NAME				= "Страна Оз";
	DBM_OZ_DESCRIPTION		= "Объявляет появления Хохотуна, Балбеса, Медноголового и Ведьмы.";
	DBM_OZ_OPTION_1			= "Показать область границы контроля в фазе 2";

	DBM_OZ_CRONE_NAME		= "Ведьма";
	DBM_OZ_YELL_DOROTHEE	= "О, Тито, нам просто надо найти дорогу домой!";
	DBM_OZ_YELL_ROAR		= "I'm not afraid a' you! Do you wanna' fight? Huh, do ya'? C'mon! I'll fight ya' with both paws behind my back!";
	DBM_OZ_YELL_STRAWMAN	= "Now what should I do with you? I simply can't make up my mind.";
	DBM_OZ_YELL_TINHEAD		= "I could really use a heart. Say, can I have yours?";
	DBM_OZ_YELL_TITO		= "Don't let them hurt us Tito! Oh, you won't, will you?";
	DBM_OZ_YELL_CRONE		= "Woe to each and every one of you, my pretties!";
	DBM_OZ_SUMMON_TITO		= "Dorothee begins to cast Summon Tito.";

	DBM_OZ_WARN_TITO		= "*** Тито ***";
	DBM_OZ_WARN_ROAR		= "*** Хохотун ***";
	DBM_OZ_WARN_STRAWMAN	= "*** Балбес ***";
	DBM_OZ_WARN_TINHEAD		= "*** Медноголовый ***";
	DBM_OZ_WARN_CRONE		= "*** Ведьма ***";

	DBM_SBT["Roar"]		= "Хохотун";
	DBM_SBT["Strawman"]	= "Балбес";
	DBM_SBT["Tinhead"]	= "Медноголовый";
	DBM_SBT["Tito"]		= "Тито";

	-- Named Beasts
	DBM_SHAD_NAME			= "Shadikith the Glider";
	DBM_HYA_NAME			= "Хиакисс Тенетник";
	DBM_ROKAD_NAME			= "Rokad the Ravager";
end
